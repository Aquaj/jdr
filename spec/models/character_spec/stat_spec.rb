...
RSpec.describe 'Character Stats', type: :model do
  ...
  let :strength { find_or_create(:strength) }
  ...
  context 'using stat_for' do
    it 'can get the base_stat value' do
      expect(zendark.base_stat_for strength).to eq 65
    end

    it 'can get the stat\'s value' do
      expect(zendark.stat_for strength).to eq 85
    end
  end

  context 'using base_stat_of' do
    it 'can get the base_stat value' do
      expect(zendark.base_stat_of strength).to eq 65
    end

    it 'can get the base stat\'s value' do
      expect(zendark.stat_of strength).to eq 85
    end
  end
end
